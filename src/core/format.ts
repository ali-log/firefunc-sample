...
export function formatMoney(
  ...
  // Derive the sign from the rounded magnitude so a value that rounds to zero
  // (e.g. -0.004 -> "0.00") has no leading "-" rather than "-$0.00".
  const sign = amount < 0 && Number.parseFloat(fixed) !== 0 ? "-" : "";
